(function($) {
        ...
        success : function(data) {
			if(!isJsonString(data)&&TAnumber&&TAnumberIndex){
				...
				if(TAnumber&&$.session.get('TAOneCity')!=1){
					...
					// domCityString = data;
					cityData(JSON.parse(data),intlCity)
				}
    ...
    KuCity.prototype = {
        ...
        creatItem: function() {
            ...
            else if(targetId == "intlArrivalCity" || this.target.context.className.indexOf("MultipleDepartureCity") != -1 || this.target.context.className.indexOf("MultipleArrivelCity") != -1){
                var city = intlCity;
            }
